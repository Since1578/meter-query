...
from PyQt5.QtWidgets import QWidget, QComboBox, QLineEdit, QListView, QApplication
...
class QComboCheckBox(QComboBox):
    ...
    def get_selected(self):
        ...
        self.items = items
        self.get_clickedcontent()
        self.get_boxstatus()
        return items

    def get_clickedcontent(self):
        if len(self.items):
            clicked = [i.text() for i in self.items]
        else:
            clicked = []
        #print(clicked)
        return clicked
    def get_boxstatus(self):
        if len(self.items):
            box_status = [i.checkState() for i in self.items]
        else:
            box_status = []
        return box_status
    ...
    # def Selectlist(self):
    #     Outputlist = [[],[],[],[]]
    #     temp=[self.year_combocheckbox,self.area_combocheckbox,self.position_combocheckbox,self.month_combocheckbox]
    #     for i in range(4):
    #         if temp[i].isChecked() == True:
    #             Outputlist[i].append(self.qCheckBox[i].text())
...
if __name__ == "__main__":
    ...
    # mainWindow = ComboCheckBox(['装置ID', '传感器ID'])
    # mainWindow.show()
    # sys.exit(app.exec_())
    app=QApplication(sys.argv)
    mainWindow = QComboCheckBox()
    mainWindow.addItems(['装置ID', '传感器ID'])
    mainWindow.show()
    sys.exit(app.exec_())